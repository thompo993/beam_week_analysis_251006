...
file_name = "all_channels_source_middle_2.npy"
...
driver_layout = ["old", "old", "old", "old", "ADA4937", "ADA4937", "ADA4937", "ADA4927"]
...
def plot_exponential_fit_and_tau(signal, tau, t_max, popt):
    ...
    plt.title("Channel %s - %s waves" %(channel, total_events)  + " - " +driver_layout[channel] + " driver")
    plt.xlabel('ns')
    plt.ylabel('LSB')
    plt.legend()
    plt.xlim(xlim)
...
def binned_average(BIN, norm = True):
    if BIN > 2**12:
        ...
        for z in range(BIN):
            ...
            reader.close_file()
        plt.title("Channel %s, number of bins: %s, " %(channel, BIN) + driver_layout[channel] + " driver")
        plt.xlabel('ns')
        if norm:
