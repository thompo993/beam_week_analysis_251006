    ...
    for folder in os.walk(FOLDER_PATH): #looks into all folders 
        ...
        for file in os.listdir(folder):
            if "%s.csv"%num in file and "amplitude" in file:
                ...
                col =""
                if int(PE)==int(folder[-3:]):
                    col = 'r'
                else: col='b'

                plt.plot(x, df, label = "Left \u0394LSB/PE = " + folder[-3:], color = col)
                plt.title("Mismatched scan, Right \u0394LSB/PE = %s - Channel %s"%(PE,num) + description)
                plt.yscale(LOG)
                if LOG == 'linear':    
                    plt.ylim(ylim)
                plt.xlim(xlim)
                ...
                if SAVE_PNG:#if true save, if false show
                    plt.savefig(os.path.join(SAVE_PATH,"CH_%s"%num+LOG+folder[-6:]))
                    plt.close()
                else: plt.show() 
